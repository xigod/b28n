function ModuleLogic(view) {
	...
	this.initValue = function (obj) {
		...
		if (!that.addInputEvent) {
			...
			if ($("#wizardPPPoEPwd_") && $("#wizardPPPoEPwd_").length > 0) {
				$(".placeholder-text").css("width", "270px")
			}
...
function ModuleView() {
	...
	this.lanIP = null;
	this.lanMask = null;
	this.connectTypes = null; //是否为无线连接
	//最后设置
	...
	function endSetup() {
		...
		$.getJSON("goform/getWizard" + getRandom(), function (obj) {
			connectInternet = obj.connectInternet; //可以上网
		});

		if (that.connectTypes == "true") {
			$("#wifiTips").show();
		} else {
			$("#wifiTips").hide();
		}
		...
		timer = setInterval(function () {
			...
			if (num == 0) {
				clearInterval(timer);
				if (connectInternet == "true") { //可以上网

					$("#successinternetWrap, #moreTips, .form-actions").show();

					if (that.connectTypes == "true") { //无线连接时隐藏more
						$(".form-actions").hide();
						$("#moreTips").hide();
					}
					$("#status").attr("class", "icon-check-circled");
				} else {
					$("#status").attr("class", "icon-warn-circled"); //不能上网时3秒后自动刷新页面
					$("#successinternetWrap, #moreTips").hide();
					if (that.connectTypes == "false") { //有线连接跳转到主页
						setTimeout(function () {
	...
	this.changePage = function (currenPage, nextPage) {
		...
		if (nextPage == "DoneWrap") {
			$("#ssid-msg").text(_("You can now connect to %s", [$("#wizardSSID").val()]));
			endSetup();
		}
	};
