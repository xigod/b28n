define(function (require, exports, module) {
	...
	function AttachedModule() {
		...
		this.initEvent = function () {
			...
			$("#qosList").delegate(".dropdown li a", "click", function () {
				if (this.parentNode.getAttribute("data-val") == "No Limit") {
					$(this).parent().parent().parent().parent().find("input[type='text']").val(_("No Limit"));
					//解决赋值问题
					$(this).parent().parent().parent().parent().find("input[type='hidden']").val(_("No Limit"));
				}
			});*/
		...
		function createOnlineList(arry) {
			...
			arry = reCreateObj(arry, "qosListIP", "up");

			var upLimit,
				downLimit;
			var hostname,
				connectType,
				localhostIP = pageModule.data.localhost || "",
				ipStr;
			...
			for (i = 0; i < len; i++) {
				if (arry[i].qosListAccess && arry[i].qosListAccess == "true") { //可以上网
					...
					for (prop in arry[i]) {
						...
						if (localhostIP == arry[i].qosListIP) {
							ipStr = arry[i].qosListIP + _("(Native Device)");
						} else {
							ipStr = arry[i].qosListIP;
						}
						if (prop == "qosListHostname") { //主机名
							str += '<td>';
							str += '<div class="col-xs-10 span-fixed deviceName"></div>';
							str += '<div class="col-xs-10 none">';
							str += ' <input type="text" class="form-control setDeviceName" value="" maxLength="32">';
							str += '</div>';
					...
					str += "</td>";
					str += "<td>" + arry[i].qosListMac.toUpperCase() + "</td>";
		...
		function updateTable(obj) {
			...
			for (i = 0; i < newLength; i++) {
				...
				for (j = 0; j < oldLength; j++) {
					...
					var $input = $tbodyList.eq(j).children().eq(0).find("input")
					if ($input[0]) {
						oldMac = $input.eq(0).attr("alt");
					} else {
						oldMac = '';
					}
		...
		function getAttacheList() {
			...
			for (i = 0; i < length; i++) {
				...
				upLimit = $td.eq(4).find(".input-append")[0].val() == ("No Limit") ? "301" : parseFloat($td.eq(4).find(".input-append")[0].val()).toFixed(2);
				if (+upLimit >= 301) {
					upLimit = 301;
				}
				str += upLimit + "\t"; //up limit

				downLimit = $td.eq(3).find(".input-append")[0].val() == ("No Limit") ? "301" : parseFloat($td.eq(3).find(".input-append")[0].val()).toFixed(2);
