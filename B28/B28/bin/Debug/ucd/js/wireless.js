define(function (require, exports, module) {
	...
	//
	pageModule.initEvent = function () {
		pageModule.update(5000, update);
	}

	function update() {
		var obj = arguments[0];
		$("#wifiChannelCurrent").text(obj.wifi.wifiChannelCurrent);
		$("#wifiBandwidthCurrent").text(obj.wifi.wifiBandwidthCurrent);
	}

	pageModule.beforeSubmit = function () {
		var wifiPwd = $("#wifiPwd").val(),
			pwd = pageModule.data.wifiPwd,
			wifiSecurityMode = $("#wifiSecurityMode").val(),
			isMobile = pageModule.data.wifiMobileDevice;

		if (pageModule.data.wifiSSID != $("#wifiSSID").val() || (wifiPwd !== pwd && wifiSecurityMode != "none")) {
	...
	function pageModuleInit() {
		this.initValue = function () {
			var type = pageModule.data.wifiRelayType;
			if (type !== "ap") {
				$("#optionWrap").hide();
			} else {
				$("#optionWrap").show();
	...
	function WifiSsidModule() {
		...
		this.initEvent = function () {
			...
			$("#wifiSecurityMode").on("change", changeSecurityMode);
	...
	function WifiParameterModule() {
		...
		this.initValue = function () {
			var obj = pageModule.data;
			createChannel(obj.wifi.wifiMaxChannel);
			createPowerView(obj.wifiPoweerGear);
			changeWifiMode(obj.wifi.wifiMode);
			inputValue(obj.wifi);
		...
		function changeWifiMode(mod) {
			...
			if (mode.indexOf("n") !== -1) {
				$("#wifiBandwidth").
				html('<option value="Auto">' + _("Auto") + '</option><option value="20">20MHz</option><option value="40">40MHz</option>');
			} else {
