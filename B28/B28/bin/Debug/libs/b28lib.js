...
exports.b28lib = function() {
    ...
    function GetPageData() {//提取html页面关键字
        ...
        function _getValue(curValue) {
            if (curValue && /\S/.test(curValue)) {
                curValue = trim(curValue);
                if (onlyZH) { //是否存在中文
                    //console.log("onlyZH curValue ===="+curValue);
                    if (/[\u4e00-\u9fa5]/.test(curValue)) {
                        nodeValueArray.push(curValue);
                    }
                } else if (/[a-z]/i.test(curValue) || /[\u4e00-\u9fa5]/.test(curValue)) {//中英文都提取
                    //console.log("not onlyZH curValue ===="+curValue);
                    nodeValueArray.push(curValue);
                } else {
                    //console.log("other onlyZH curValue ===="+curValue);
    ...
    function GetXMLData(data, file) {
        var ret = [];

        data.replace(/msgid="(.*?)"/g, function(matches) {
            matches = matches.replace(/msgid="*"/g, '');
            ret.push(matches.substring(0, matches.length - 1))
        });
        //console.log("aaaaaaaacccc "+ret);
        return ret;
    }
