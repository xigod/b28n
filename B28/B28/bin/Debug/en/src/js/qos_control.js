        ...
        if ($("[name=qosmode]:checked").val() == "0") {
            ...
            if ($("#apWidth").val() != "") {
                ...
                if (t.test($("#apWidth").val()) == false) {
                    alert(_("The total AP bandwidth must be an integer."));
                    return false;
                } else if (parseFloat($("#apWidth").val()) < 0.1 || $("#apWidth").val() > 100) {
                    alert(_("Please specify a valid total AP bandwidth from 0.1 through 100."));
                    return false;
                }
            } else {
                // alert(_("Please specify the total AP bandwidth."));
                // return false;
            }
            if ($("#intUpWidth").val() != "") {
                if (t.test($("#intUpWidth").val()) == false) {
                    alert(_("The maximum upload rate must consist of digits."));
                    return false;
                } else if (parseFloat($("#intUpWidth").val()) < 0.1 || $("#intUpWidth").val() > 100) {
                    alert(_("Please specify a valid maximum upload rate from 0.1 through 100."));
                    return false;
                }
            } else {
                // alert(_("Please specify the maximum upload rate."));
                // return false;
            }

            if ($("#intDownWidth").val() != "") {
                if (t.test($("#intDownWidth").val()) == false) {
                    alert(_("The maximum download rate must consist of digits."));
                    return false;
                } else if (parseFloat($("#intDownWidth").val()) < 0.1 || $("#intDownWidth").val() > 100) {
                    alert(_("Please specify a valid maximum download rate from 0.1 through 100."));
                    return false;
                }
            } else {
                // alert(_("Please specify the maximum download rate."));
                // return false;
            }
        } else {
            $("#apWidth").removeAttr("name");
            ...
            if ($("#userSelfUpWidth").val() != "") {
                if (t.test($("#userSelfUpWidth").val()) == false) {
                    alert(_("The maximum user upload rate must consist of digits."));
                    return false;
                } else if (parseFloat($("#userSelfUpWidth").val()) < 0.1 || $("#userSelfUpWidth").val() > 100) {
                    alert(_("Please specify a valid maximum user upload rate from 0.1 through 100."));
                    return false;
                }
            } else {
                // alert(_("Please specify the maximum user upload rate."));
                // return false;
            }

            if ($("#userSelfDownWidth").val() != "") {
                if (t.test($("#userSelfDownWidth").val()) == false) {
                    alert(_("The maximum user download rate must consist of digits."));
                    return false;
                } else if (parseFloat($("#userSelfDownWidth").val()) < 0.1 || $("#userSelfDownWidth").val() > 100) {
                    alert(_("Please specify a valid maximum user download rate from 0.1 through 100."));
                    return false;
                }
            } else {
                // alert(_("Please specify the maximum user download rate."));
                // return false;
            }
        }
