...
$(function() {
    var logadd = {
        ...
        validate: function() {
            ...
            if (!checkIp(f.innerIP, _("Log Server IP Address"))) {
                return false;
            }
            f.innerIP.value = clearInvalidIpstr(f.innerIP.value);

            if (!t.test(+f.port.value) || +f.port.value < 1 || +f.port.value > 65535) {
                alert(_("Please specify a valid port number from 1 through 65535."));
                return false;
            }
            if (f.innerIP.value == lanip) {
                alert(_("The log server IP address cannot be the same as the default gateway IP address."));
                return false;
            }
