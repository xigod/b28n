...
$(function () {
				...
				if (f.workmode[2].checked == true) {
					...
					if (f.wrlRadio.value == '2.4G' && macstr != "") {
						...
						for (var i = 0; i < tempStatus.length; i++) {
							if (tempStatus[i] == 'Up') {
								$("#macSta" + (i + 1)).html(_(_("Connected"))).attr("class", macClass[0]);
							} else if (tempStatus[i] == "Down") {
								$("#macSta" + (i + 1)).html(_(_("Disconnected"))).attr("class", macClass[1]);
							} else {
								$("#macSta" + (i + 1)).html(_("Unknown"));
							}
						}
	...
	if (bodyId == "basic") {
		var initValue = function () {
			...
			$.getJSON("/goform/wifiSSIDget?index=" + ssidIndex + "&wrlRadio=" + f.wrlRadio.value, function (str) {
				...
				if (curmode == 1 && 0 == ssidIndex) {
					alert(_("In WDS mode, the settings for the first SSID cannot be changed."))
		...
		var helpStr = "";
		helpStr = _(_("(Range: 1 to %s)"), [maxClientNum]);
		...
		$("input:[name=enable]").change(function () {
			if (this.checked == false && f.index.value == 0) {
				if (curmode == 1) {
					alert(_("In WDS mode, the first SSID cannot be disabled. Otherwise, the WDS function fails."));
					this.checked = true;
				}
				if (curmode == 2) {
					alert(_("In Client+AP mode, the first SSID cannot be disabled. Otherwise, the Client+AP function fails."));
					this.checked = true;
				}
			}
		});
	...
	if (bodyId == "radio") {
		...
		$.getJSON("/goform/WifiRadioGet?r=" + Math.random() + "radio=" + f.wrlRadio.value, function (str) {
			...
			if (curmode == 1 || curmode == 2) {
				alert(_("In Client+AP mode or WDS mode, the settings on this page cannot be changed."))
				$("#btnsubmit").attr("disabled", true);
			}
	...
	if (bodyId == "wds") {
		...
		var wdsModeOnChange = function () {
				if (f.wds_mode.selectedIndex == 0) {
					...
					document.getElementById("wlScan").value = _("Scan");
				}
			},
			wdslistArray;
			...
			$.get("/goform/WifiClientList?r=" + Math.random() + "&index=" + ssidIndex + "&wrlRadio=" + f.wrlRadio.value, function (str) {
				...
				if (datas.length == 1) {
					...
					for (i = 0; i < datas.length - 1; i++) {
						...
						tbd.rows[i].insertCell(2).innerHTML = datas[i].split("\t")[1] != "0.0.0.0" ? datas[i].split("\t")[1] : _("Unknown");
	...
	if (this.value == _("Scan")) {
		...
		$.get("/goform/wifiWDS?radio=" + f.wrlRadio.value + "&" + Math.random(), function (data) {
			...
			var str1 = data.split("\r"),
				...
				'<td width="130">' + _("Security Mode") + '</td>' +
...
function macSel(th) {
	if (!confirm(_("Do you want to connect to this AP?"))) {
		return;
	}
...
function CheckValue() {
	...
	if (document.body.id == "mode") {
			...
			for (i = 1; i < 5; i++) {
				for (j = i + 1; j < 5; j++) {
					if (f["wdsMac" + i].value != "" && f["wdsMac" + i].value == f["wdsMac" + j].value) {
						alert(_("The MAC address exists."));
						f["wdsMac" + j].value = "";
						return false;
					}
				}
		...
		if (f.workmode[3].checked == true) {
			...
			var mtu_reg = /^[0-9]{1,}$/;
			if (ppoeUser == "" || ppoePwd == "") {
				alert(_("Please enter a PPPoE user name or password."));
				return false;
			}
			if (/[\\]/g.test(ppoeUser) || /[\\]/g.test(ppoePwd)) {
				alert(_(_("%s is not allowed in the PPPoE user name and password."), [""]));
				return false;
			}
			if (ppoeMtu == "" || !mtu_reg.test(ppoeMtu) || (ppoeMtu > 1492) || (ppoeMtu < 256)) {
				alert(_("Please enter an MTU value from 256 through 1492."));
				return false;
			}
		}
		return true;
	...
	if (document.body.id == "basic") {
		...
		for (var i = 0; i < ssid_arry.length; i++) {
			if (f.ssid.value == decodeSSID(ssid_arry[i]) && i != f.index.value) {
				alert(_("The SSID cannot be the same as another of this device for the same frequency band."));
				return false;
			}
		}
		...
		/*
		if (tmp > 128) {
			alert(_("A maximum of 128 clients are allowed for all enabled SSIDs."));
			return false;
		}
		*/

		if (!checkData(f.maxclients, _("Max. Number of Clients"), 1, maxClientNum)) {
			return false;
		}
		if (f.enable.checked == true) {
	...
	if (document.body.id == "qvlan") {
		...
		if (f.qvlanEn.checked == true) {
			...
			if (!checkData(f.manage_vlan, _("QVLAN"), 1, 4094)) {
				return false;
			}

			/*
			if (qvlanIndex == maxSsidNum) {
				alert(_("VLAN IDs must be different."));
				return false;
			}
			*/
		} else {
	...
	if (document.body.id == "qvlan_5g") {
		...
		if ((qvlan_en == 0 && f.qvlanEn.checked == false) ||
			...
			alert(_("The QVLAN settings are not changed. You do not need to save them."));
			return false;
		}
		if (f.qvlanEn.checked == true) {
			f.qvlanEnlan.value = 1;
	...
	if (document.body.id == "wds") {
		...
		if (enablewireless == 0 && f.wrlRadio.value == '2.4G') {
			alert(_("Please enable the 2.4 GHz network."));
			return false;
		}
		if (enablewireless == 0 && f.wrlRadio.value == '5G') {
			alert(_("Please enable the 5 GHz network."));
			return false;
		}
	...
	if (document.body.id == "filter") {
		...
		if (flist.length == 0 && f.filterMode.selectedIndex == 2) {
			//alert(_("You must configure at least one rule if MAC Address Filter Mode is set to Disallow."));
			//return false;
		}
...
function preSubmit() {
	...
	if (bodyId == "mode") {
		if (enableCliNum > 128) {
			alert(_("When the settings are saved, the first SSID will be enabled and the total number of clients of the enabled SSIDs will exceed the upper limit, which is 128. Please change the upper limit on the Wireless > SSID page."));
			return false;
		}
		if (f.workmode[0].checked == true) {
			if (f.wrlRadio.value == '2.4G') {
