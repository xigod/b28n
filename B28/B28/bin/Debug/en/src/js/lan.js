...
function preSubmit() {
	...
	if (initObj.lanMode == lanMode && initObj.lanIp == lanIp && initObj.lanMask == lanMask && initObj.lanGw == lanGw && lanDns1 == initObj.lanDns1 && lanDns2 == initObj.lanDns2 && initObj.ethMode == f.ethMode.value) {
		...
		if (initObj.apName == f.apName.value) {
			alert(_("You have already saved the settings."));
			return false;
		} else {
			f.reboot.value = "0";
		}
	...
	if (lanMode == "static") { //静态IP	
		...
		if (!checkIp(f.lanGw, _("Default Gateway"))) {
			return;
		}
		...
		if (!checkVerifyIp(lanIp, lanMask)) {
			alert(_("The host field of the IP address cannot be 0."));
			return false;
		}
		...
		if (lanIp.split(".")[3] == "255") {
			alert(_("The last field in the IP address cannot be 255."));
			return false;
		}
		...
		if (lanIp == lanGw) {
			alert(_("The IP address cannot be the same as that of the default gateway."));
			return false;
		}
