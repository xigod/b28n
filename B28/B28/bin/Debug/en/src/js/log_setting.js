    ...
    logset = {
        ...
        initList: function() {
            ...
            strtmp += '<thead class="a4"><tr>' +
                '<td width="50">ID</td><td width ="180">' + _("Log Server IP Address") + '</td>' +
                '<td width ="120">' + _("Log Server Port") + '</td><td width ="100">' + _("Enable") + '</td><td>' + _("Operation") + '</td>' +
        ...
        initEvent: function() {
            $("#addToList").on('click', function() {
                if (entrynum >= 4) {
                    alert(_("Only a maximum of 4 log servers are allowed."));
                } else {
        ...
        validate: function() {
            if (isNaN(f.logsNum.value) || f.logsNum.value > 300 || f.logsNum.value < 100) {
                alert(_("Incorrect number of logs."));
                return false;
            }
        ...
        onDel: function(index) {
            if (window.confirm(_("Do you want to delete the rule?"))) {
