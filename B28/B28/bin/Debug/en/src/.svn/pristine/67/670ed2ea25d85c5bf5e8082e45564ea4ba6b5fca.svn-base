...
G.MSG = {
	userEmpty: _("Please specify the user name!"),
	userLenErr: _("1~32 characterts are allowed in the user name."),
	pwdEmpty: _("Please specify the password!"),
	pwdLenErr: _("1~32 characterts are allowed in the password."),
	userOrpwdErr: _("User name or password error!"),
	pwdErr: _("Password error")
};
var login = {
...
function Login(Encode) {
	...
	this.initHtml = function () {
		...
		if (G.data.hasUser) {
			...
			$("#" + G.data.user).addPlaceholder(_("The default username is admin"));
		...
		$("#" + G.data.password).val("");
		$("#" + G.data.password).initPassword(_("The default password is admin"));
	};
