            ...
            for (var i = 0; i < 4; i++) {
                for (var j = 1; j < 6; j++) {
                    ...
                    if (apValue.length == 0 || apValue < 0 || apValue > 65535) {
                        if (j == 1) {
                            alert(_("CWmin error. The valid range is 0~65535."));
                            $("#apwmm").find("tr").eq(i).find("td").eq(j).find("input").focus();
                            return false;
                        } else if (j == 2) {
                            alert(_("CWmax error. The valid range is 0~65535."));
                            $("#apwmm").find("tr").eq(i).find("td").eq(j).find("input").focus();
                            return false;
                        } else if (j == 3) {
                            alert(_("AIFSN error. The valid range is 0~65535."));
                            $("#apwmm").find("tr").eq(i).find("td").eq(j).find("input").focus();
                            return false;
                        } else if (j == 5 && f.wrlRadio.value == '2.4G') {
                            alert(_("TXOP Limit error. The valid range is 0~65535."));
                            $("#apwmm").find("tr").eq(i).find("td").eq(j).find("input").focus();
                            return false;
