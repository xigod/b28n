...
function initValue(obj) {
	...
	if (datas.length == 0) {
		if (obj.osEnable) {
			$(tbd).html("<tr><td colspan=8>" + _("No clients connected!") + "</td></tr>");
		} else {
			$(tbd).html("<tr><td colspan=7>" + _("No clients connected!") + "</td></tr>");
		}
	} else {
		$("#list tr").remove();
		for (i = 0; i < datas.length; i++) {
			...
			dataStr += "<td>" + (datas[i].ip != "0.0.0.0" ? datas[i].ip : _("Unknow")) + "</td>";
