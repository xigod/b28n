...
function initHTML() {
    var data = null;
    var str = "<li class='a3'><span style='margin-left: 10px;'>" + _("URL Sets") + "</span><label>" + _("Action") + "</label></li><li class='uGroup' tindex='1'><label style='float:left;width:140px;margin-right: 0px;cursor:pointer;' name='gNameSpan' title='"+_("Shopping")+"'><span style='margin-left: 10px;'>" + _("Shopping") + "</span></label><label><input type='checkbox' name='enable'>" + _("Enable") + "</label></li><li class='uGroup' tindex='2'><label name='gNameSpan' title='"+_("group-buying")+"' style='float:left;width:140px;margin-right: 0px;cursor:pointer;'><span style='margin-left: 10px;'>" + _("group-buying") + "</span></label><label><input type='checkbox' name='enable'>" + _("Enable") + "</label></li><li class='uGroup' tindex='3'><label name='gNameSpan' title='"+_("Video")+"' style='float:left;width:140px;margin-right: 0px;cursor:pointer;'><span  style='margin-left: 10px;'>" + _("Video") + "</span></label><label><input type='checkbox' name='enable'>" + _("Enable") + "</label></li><li class='uGroup' tindex='4'><label name='gNameSpan' title='"+_("Chatting Email")+"' style='float:left;width:140px;margin-right: 0px;cursor:pointer;'><span style='margin-left: 10px;'>" + _("Chatting Email") + "</span></label><label><input type='checkbox' name='enable'>" + _("Enable") + "</label></li><li class='uGroup' tindex='5'><label name='gNameSpan' title='"+_("Recruit")+"' style='float:left;width:140px;margin-right: 0px;cursor:pointer;'><span style='margin-left: 10px;'>" + _("Recruit") + "</span></label><label><input type='checkbox' name='enable'>" + _("Enable") + "</label></li>";
...
function initEvent() {
    ...
    $("#addMenu").on('click', function() {
        if (G.groupLen >= 10) {
            alert(_("Up to 5 new categories can be added."));
            return false;
        } else {
            $("#addGroupUL").show();
            ...
            if (addGroupName == "") {
                ...
                for (var i = 5; i < G.groupName.length; i++) {
                    if (addGroupName == G.groupName[i]) {
                        alert(_("The user group name already exists."));
                        return false;
                    }
                }
    ...
    $("#groupNav").delegate('span:[name=delete]', 'click', function(e) {
        if (permission()) {
            ...
            // if (!confirm(_("Delete this category?"))) {
            //     return false;
            // } else {
                ...
                if ($(this).parents("li").find('input').is(":checked")) {
                    confirmMsg = _("This group is enabled and can't be deleted!");
                    alert(confirmMsg);
                    return;
                } else {
                    if (!confirm(_("Delete this category?"))) {
    ...
    $("#addUrlMenu").on('click', function() {
        ...
        if (G.urlLen >= 10) {
            alert(_("Up to 10 URLs can be added!"));
            return false;
        } else {
            if (G.urlLen == 0) {
                $("#noRuleMsg").hide();
    ...
    $("#speContent").delegate(".save_icon", 'click', function() {
        if (permission()) {
            ...
            if (G.urlLen >= 10) {
                ...
                if (description == "") {
                    alert(_("Please enter the URL description."));
                    return false;
                } else if (url == "") {
                    alert(_("Please enter the URL link."));
                    return false;
...
function showUrlGroupList(name) {
    ...
    $.post('./goform/getUrls', 'urlGroupName=' + name + '&rd=' + Math.random(), function(res) {
        ...
        if (data.length == 0) {
            str1 = "<tr style='text-align:center;' id='noRuleMsg'><td colspan='4'>" + _("No URL exists yet.") + "</td></tr>";
...
function validateCheck(type, str) {
    ...
    if (type == "groupName" || type == "des") {
        ...
        if (length > 32) {
            if (type == "groupName") {
                alert(_("The max length of new category name is 32 bytes."));
            } else {
                alert(_("The max length of the description is 32 bytes."));
            }
            return false;
        } else if (!t.test(str)) {
            if (type == "groupName") {
                alert(_("Only numbers, alphabets and underlines can be allowed in the new category name."));
            } else {
                alert(_("Only numbers, alphabets and underlines can be allowed in the URL description."));
            }
            return false;
        } else {
    ...
    if (type == "url") {
        if (str.length > 64) {
            alert(_("The max length of the new URL is 64 bits."));
            return false;
        } else if (!h.test(str)) {
            alert(_("Only numbers, alphabets and '.' can be allowed in the new URL."));
            return false;
        } else if (f.test(str)) {
            if (/.+\..+/ig.test(str) || str == "localhost") {
                return true;
            } else {
                alert(_("Invalid URL format!"));
                return false;
            }
        } else {
            alert(_("Invalid URL format!"));
